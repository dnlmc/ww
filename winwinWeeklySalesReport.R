...
rownames(lwtable)<-c("Bar - Beer/Wine", "Bar - Liquor", "Coffee", "Kitchen", "None", "Discounts", "Total")
...
plot(as.Date(weeklyframe$weekending, origin = "1970-01-01"), weeklyframe$net, type = "l", col = "green",
...
legend("topright", legend= c("goal", "total", "beer/wine", "liquor",
       ...
       lwd = 2, x.intersp = 1, y.intersp = .90, inset=c(.015, .0025))
...
text(cex=.95, x=xx+.2, y=-3, bblabs, xpd=TRUE, srt=55, pos=2)
...
text(cex=.95, x=xy+.33, y=-1, bllabs, xpd=TRUE, srt=55, pos=2)
...
text(cex=1, x=x+.20, y=-.60, labs, xpd=TRUE, srt=50, pos=2)
